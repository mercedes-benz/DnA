...
import Styles from './recipes-tab.scss';
...
import RecipeList from '../manageRecipes/RecipeList';
...
const RecipesTab = () => {
  ...
  const getAllRecipes = () => {
    ...
    setLoading(true);
    CodeSpaceApiClient.getAllRecipes()
      .then((res) => {
        setLoading(false);
  ...
  return (
    <React.Fragment>
      <button className={classNames('btn btn-primary', Styles.actionBtn)} onClick={() => history.push(`/codespaceRecipes`)}>
      ...
      <div className={Styles.content}>
        <div>
          <div>
            {!loading && (
              recipes?.length === 0 &&
                <div className={Styles.empty}>Recipes are not available</div>
            )}
