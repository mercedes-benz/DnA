...
export const serializeReportRequestBody = (requestBody: ICreateNewReportRequest) => {
  ...
  requestBody.data.description.frontendTechnologies = requestBody.data.description.frontendTechnologies?.length > 1 ? requestBody.data.description.frontendTechnologies
    ?.filter((item: IFrontEndTech) => item.name !== 'Choose')
    ?.map((item: IFrontEndTech) => item.name) as any[] : null;
  requestBody.data.description.agileReleaseTrains = requestBody.data.description.agileReleaseTrains?.length > 0 ? requestBody.data.description.agileReleaseTrains?.map(
    (item: IART) => item.name,
  ) as any[] : null;
  requestBody.data.description.integratedPortal = requestBody.data.description.integratedPortal?.length > 0 ? requestBody.data.description.integratedPortal?.map(
    (item: IIntegratedPortal) => item.name,
  ) as any[] : null;
  requestBody.data.description.designGuideImplemented = requestBody.data.description.designGuideImplemented?.length > 1 ? requestBody.data.description.designGuideImplemented
    ?.filter((item: IDesignGuide) => item.name !== 'Choose')
    ?.map((item: IDesignGuide) => item.name)
    ?.toString() as any : null;
  requestBody.data.description.productPhase = requestBody.data.description.productPhase?.length > 1 ? requestBody.data.description.productPhase
