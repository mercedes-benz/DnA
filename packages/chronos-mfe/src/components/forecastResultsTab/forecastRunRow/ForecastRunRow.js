    ...
    <React.Fragment>
      ...
      >
        ...
        <td>
          ...
          {(item.warnings !== null && item.warnings.length === 0 && item.warningsInfo !== null && item.warningsInfo.length > 0) || (item.state.result_state === 'INFO') && <i className={classNames('icon mbc-icon info circle', Styles.infoCircle)}  onClick={(e) => handleStatusClick(e, item)} tooltip-data={'Click to View the Info'} />}
