...
// A regex to validate that user is setting a Short ID
const SHORTID_RGX = /^\w{6,8}$/;
...
export class WorkgroupApi {
    ...
    async handleContributor(action: ContributorActions, req: Request, res: Response) {
        ...
        if (!SHORTID_RGX.test(contributor)) {
            return apiError({
                res,
                error: `Contributor doesn't look like a valid short ID`,
            });
        }
        let errIndex = 0;
        try {
            const binding = mapSimpleBindingToWorkgroupBinding({
                user: contributor.toUpperCase(),
                namespace,
                role: 'contributor',
            });
            const {headers} = req;
