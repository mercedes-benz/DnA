...
Refer the airflow section in the [values.yaml](../deployment/kubernetes/helm/values.yaml) for more info on each parameter.
...
Build the images of the notebook service using the below command
```
cd documents
docker-compose -f docker-compose-notebook.yml build
```
update the images names in the [values.yaml](../deployment/kubernetes/helm/values.yaml) . You can refer the values.yaml to check on where to update the image names.

Update the configproxyauthtoken value in the [values.yaml](../deployment/kubernetes/helm/values.yaml) and this can be generated by using the "openssl rand -hex 32"
```
app:
  jupyter:
    config:
