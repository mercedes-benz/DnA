    ...
    <React.Fragment>
      ...
      >
        ...
        <td>
          ...
          {((item.state.result_state === 'INFO') || (item.warnings !== null && item.warnings.length === 0 && item.warningsInfo !== null && item.warningsInfo.length > 0)) && <i className={classNames('icon mbc-icon info circle', Styles.infoCircle)}  onClick={(e) => handleStatusClick(e, item)} tooltip-data={'Click to View the Info'} />}
          ...
          {(item.state.result_state === 'SUCCESS' || item.state.result_state === 'WARNINGS' || item.state.result_state === 'INFO') && item.backtesting !== undefined && item.backtesting !== null && item.backtesting !== '' && <span className={Styles.comparisonIcon}><i className={classNames('icon mbc-icon data-sharing')}  onClick={(e) => { e.stopPropagation(); props.runComparison(item.id); }} tooltip-data={'Click to Run Comparison'} /></span>}
        </td>
        <td>
