...
export const serializeReportRequestBody = (requestBody: ICreateNewReportRequest) => {
  ...
  requestBody.data.description.frontendTechnologies = requestBody.data.description.frontendTechnologies
    ?.filter((item: IFrontEndTech) => item.name !== 'Choose')
    ?.map((item: IFrontEndTech) => item.name) as any[];
  requestBody.data.description.agileReleaseTrains = requestBody.data.description.agileReleaseTrains?.map(
    (item: IART) => item.name,
  ) as any[];
  requestBody.data.description.integratedPortal = requestBody.data.description.integratedPortal?.map(
    (item: IIntegratedPortal) => item.name,
  ) as any[];
  requestBody.data.description.designGuideImplemented = requestBody.data.description.designGuideImplemented
    ?.filter((item: IDesignGuide) => item.name !== 'Choose')
    ?.map((item: IDesignGuide) => item.name)
    ?.toString() as any;
  requestBody.data.description.productPhase = requestBody.data.description.productPhase
