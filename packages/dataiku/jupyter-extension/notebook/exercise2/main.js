...
define([
    ...
    */
    ...
    function get_first_cell_after_hidden(cell) {
        while (cell !== null && cell.metadata.solution2 !== undefined) {
    ...
    function click_solution_lock(evt) {
        ...
        // increase counter only when the answer is being revealed not hidden
        if (is_locked) {
            cnt_solution += 1;
        }
        console.log("Number of revealed solutions: %i", cnt_solution);
    ...
    var no_submission = 0;
    function submit_result(cell) {
        no_submission += 1;
        console.log('Number of submisions: %s', no_submission);
        var user_solution = get_first_cell_after_hidden(cell);
        if (user_solution !== null) {
            console.log(user_solution);
            var user_result = user_solution.output_area.outputs[0].data["text/plain"];
            console.log("User solution: %s \nUser result: %s", user_solution.get_text(), user_result);
        }
        else {
            console.log("No cell with solution");
        }
    }
    ...
    function add_element(cell) {
        ...
        ctrl = $([
            ...
            '  </div>',
            '  <div class="onoffswitch">',
            '    <input class="onoffswitch-submit" type="submit" id="mySubmit">',
            '  </div>',
            '</div>'
        ].join('\n'))
            .appendTo(cell.element);
        ctrl.find('input.onoffswitch-checkbox')
            .on('click', click_solution_lock);
        ctrl.find('input.onoffswitch-submit')
            .on('click', submit_result.bind(null, cell));
