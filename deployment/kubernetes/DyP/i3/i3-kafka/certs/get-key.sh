...
kubectl get secret dna-user  -n <<namespace>>  -o jsonpath='{.data.user\.key}' | base64 -d > user.key
kubectl get secret dna-user -n <<namespace>> -o jsonpath='{.data.user\.crt}' | base64 -d > user.crt
kubectl get secret my-cluster-cluster-ca-cert -n <<namespace>> -o jsonpath='{.data.ca\.crt}' | base64 -d > ca.crt
