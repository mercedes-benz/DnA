...
public class TrinoDataLakeCustomRepoImpl extends CommonDataRepositoryImpl<TrinoDataLakeNsql, String>
	...
	public Long getCountByUserAndProject(String userId, String id) {
		String query = "select count(*) from trino_datalake_nsql where  id = '"+ id + "' and ( jsonb_extract_path_text(data,'createdBy','id') = '" + userId + "' or jsonb_extract_path_text(data,'tables') ~* '" + userId + "')";
		Query q = em.createNativeQuery(query);
		BigInteger results = (BigInteger) q.getSingleResult();
		return results.longValue();
	}
