...
public class TrinoDataLakeCustomRepoImpl extends CommonDataRepositoryImpl<TrinoDataLakeNsql, String>
	...
	public List<TrinoDataLakeNsql> getAll(String userId, int offset, int limit){
		String getAllStmt = " select cast(id as text), cast(data as text) from trino_datalake_nsql where jsonb_extract_path_text(data,'createdBy','id') = '" + userId + "' or jsonb_extract_path_text(data,'collabs') ~* '" + userId + "'";
		if (limit > 0)
	...
	public Long getCount(String userId) {
		String query = "select count(*) from trino_datalake_nsql where jsonb_extract_path_text(data,'createdBy','id') = '" + userId + "' or jsonb_extract_path_text(data,'collabs') ~* '" + userId + "'";
	...
	public Long getCountByUserAndProject(String userId, String id) {
		String query = "select count(*) from trino_datalake_nsql where  id = '"+ id + "' and ( jsonb_extract_path_text(data,'createdBy','id') = '" + userId + "' or jsonb_extract_path_text(data,'collabs') ~* '" + userId + "')";
		Query q = em.createNativeQuery(query);
		BigInteger results = (BigInteger) q.getSingleResult();
		return results.longValue();
	}
