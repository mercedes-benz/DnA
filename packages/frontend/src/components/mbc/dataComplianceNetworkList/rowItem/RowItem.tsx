...
const RowItem:React.FC<IRowItemProps> = (props: IRowItemProps) => {
  ...
  return (
    <tr id={item.id} className="data-row">
      ...
      <td className="wrap-text">{item.localComplianceOfficer !== undefined && (item.localComplianceOfficer.length > 0 ? item.localComplianceOfficer.map((item:any) => item + '; ') : 'N/A')}</td>
